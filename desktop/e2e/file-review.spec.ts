...
test("file mutation tool filename opens the right sidebar unified review diff", async ({ page }) => {
  ...
  await expect(wrapButton).toHaveAttribute("data-tooltip-label", "自动换行");
  await expect(page.getByLabel("文件 diff")).toHaveAttribute("data-wrap", "false");
  await wrapButton.hover();
  await expect(page.locator("[role='tooltip']").filter({ hasText: "自动换行" })).toBeVisible();
  await wrapButton.click();
  await expect(page.getByRole("button", { name: "关闭自动换行" })).toHaveAttribute("data-tooltip-label", "取消换行");
